...
<body>
    ...
    <?php
    $sql="select * from marks where cid='$cid' and testname='$testname' order by cid";
    $query=mysqli_query($conn, $sql);
    ?>
    <form action="markupdatedmarks.php" method="POST">
    ...
    while($row=mysqli_fetch_array($query)){
        ...
        ?>
            <tr>
                ...
                <td><input type="number" name="<?php echo $row['susername']; ?>" value="<?php echo $row['marks']; ?>" required></td>
            </tr>
        <?php
    }
    ?>
    ...
    if(isset($_POST['submit']))
    {
        ...
        while($row=mysqli_fetch_array($query)){
            ...
            $sname=$row1['name'];
            $marks=$_POST["$sid"];
            $update="update marks set marks='$marks' where cid='$cid' and testname='$testname' and susername='$sid';";
            mysqli_query($conn, $update);
        }
        echo "<div style='color:green;text-shadow:0px 0px 2px;text-align:center;'>Marks Updated</div>";
        ?>
